...
start "Backend" cmd /k "set ASPNETCORE_URLS=http://localhost:5080 && dotnet run --project backend\src\Api\Api.csproj --no-launch-profile"

echo Waiting a moment before launching frontend...
timeout /t 2 /nobreak >nul

echo Starting frontend in a new window...
start "Frontend" cmd /k "cd frontend && set VITE_API_BASE_URL=http://localhost:5080 && npm run dev -- --host"
